...
///////////////////////////////////////////////////////////////////////////////////////////
function OnGUI () // VISSUAL 
{
    GUI.Label(Rect(900, 20, 300, 300), String.Format("Torque: "+ "{0:0.}", CurSpeed));
    GUI.Label(Rect(900, 47, 300, 300), String.Format("Torque: "+ Torque));  
    GUI.Label(Rect(900, 74, 300, 300), String.Format("SteerAngle: "+ SteerAngle));
    GUI.Label(Rect(900, 101, 300, 300), String.Format("AngularDrag: "+ GetComponent.<Rigidbody>().angularDrag));
}   
///////////////////////////////////////////////////////////////////////////////////////////
function Start ()
{
...
function FixedUpdate ()
{
   ...
   if (CurSpeed > 50 && WheelF.isGrounded)
		{
			GetComponent.<Rigidbody>().angularDrag = 8;
		}	
	...
	if (WheelF.isGrounded && WheelR.isGrounded)
	   {
   			if ( Vector3.Angle( Vector3.up, transform.up ) > 45 && Input.GetAxis("Horizontal") ) 
   	 			 {
  					GetComponent.<Rigidbody>().AddRelativeTorque (Vector3.forward * 5000 * Input.GetAxis("Horizontal"));
      			 }
      		else {  GetComponent.<Rigidbody>().angularDrag = 8;  }	 
	   }  
	// FUERZA MIENTRAS ESTA EN EL AIRE
	else 
		{	       
			if (Input.GetKey (KeyCode.W))
				{   	GetComponent.<Rigidbody>().AddRelativeTorque (Vector3.right * 1500 * Input.GetAxis("Vertical"));          		}
			if (Input.GetKey (KeyCode.S))
				{  		GetComponent.<Rigidbody>().AddRelativeTorque (Vector3.left * -1500 * Input.GetAxis("Vertical"));    	        	}   
			if (Input.GetKey (KeyCode.D)) 
				{		GetComponent.<Rigidbody>().AddRelativeTorque (Vector3.back * 550 * Input.GetAxis("Horizontal")); 		        }
			if (Input.GetKey (KeyCode.A))
				{		GetComponent.<Rigidbody>().AddRelativeTorque (Vector3.forward * -550 * Input.GetAxis("Horizontal")); 		}	    	
		}  
...
function Update () 
{
   ...
   CurSpeed = GetComponent.<Rigidbody>().velocity.magnitude*3.0;
